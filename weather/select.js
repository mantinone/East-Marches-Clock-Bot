...
function table_roll(a){
  return a=="d6pct"
    ? rd_dice(1,6)*10+rd_dice(1,6)
    : parseInt(roll_dice(a))
}

function table_result(a,b){
  var c;
  Object.keys(a).each(function(e){
    var f,g;if(match=/(\d+)-(\d+)/.exec(e)){
      f=cv(match[1]);g=cv(match[2])
    }else if(match=/(\d+)/.exec(e))g=f=cv(match[1]);
    if(match&&b>=f&&b<=g)c=a[e]
  });
  if(c){
    if(c.dice){
      var d=table_roll(c.dice);c=table_result(c,d)
    }
    if(/\d+d\d+/.exec(c))c=roll_dice(c)}
    else c="No result ("+b+")";
    return c
}
...
function scale_table(a){
  Object.keys(a).each(function(b){if(match=/(\d+)-(\d+)/.exec(b)){min=cv(match[1]);max=cv(match[2])}else if(match=/(\d+)/.exec(b))max=min=cv(match[1]);if(match){if(Object.isUndefined(a.min))a.min=min;else if(min<a.min)a.min=min;if(Object.isUndefined(a.max))a.max=max;else if(max>a.max)a.max=max}});return a}function d6pct_mod(a,b){a=(Math.floor(a/10)-1)*6+a%10;a+=table.res.mod;if(a<1)a=1;else if(a>36)a=36;return(Math.floor((a-1)/6)+1)*10+(a-1)%6+1}
